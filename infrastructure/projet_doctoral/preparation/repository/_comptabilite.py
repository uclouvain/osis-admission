...
def get_dto_accounting_from_admission(admission: DoctorateAdmission) -> ComptabiliteDTO:
    accounting = getattr(admission, 'accounting', None)

    if accounting:
        fr_study_allowance_application = accounting.french_community_study_allowance_application
        parent_25_26_annex = accounting.parent_refugees_stateless_annex_25_26_or_protection_decision

        return ComptabiliteDTO(
            attestation_absence_dette_etablissement=accounting.institute_absence_debts_certificate,
            demande_allocation_d_etudes_communaute_francaise_belgique=fr_study_allowance_application,
            enfant_personnel=accounting.is_staff_child,
            attestation_enfant_personnel=accounting.staff_child_certificate,
            ...
            titre_identite_sejour_longue_duree_ue=accounting.ue_long_term_stay_identity_document,
            titre_sejour_belgique=accounting.belgium_residence_permit,
            affiliation_sport=accounting.sport_affiliation,
            etudiant_solidaire=accounting.solidarity_student,
    ...
    return get_dto_accounting_from_domain_model(comptabilite_non_remplie)
