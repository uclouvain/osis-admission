...
class DemandeRepository(IDemandeRepository):
    ...
    def _load_dto(cls, demande: DemandeProxy) -> DemandeDTO:
        ...
        countries = {
            country.get('iso_code'): country.get(field_to_retrieve)
            for country in Country.objects.filter(iso_code__in=[residential_country, country_of_citizenship]).values(
                'iso_code',
                field_to_retrieve,
            )
        }
