...
from admission.ddd.doctorat.domain.service.i_notification import INotification
...
class Notification(INotification):
    ...
    def envoyer_message(
        ...
        if cc_promoteurs:
            for promoter in actors.filter(type=ActorType.PROMOTER.name):
                cc_list.append(cls._format_email(promoter.person))
        if cc_membres_ca:
            for ca_member in actors.filter(type=ActorType.CA_MEMBER.name):
                cc_list.append(cls._format_email(ca_member.person))
        if cc_list:
    ...
    @classmethod
    def _format_email(cls, person: Person):
        return "{p.first_name} {p.last_name} <{p.email}>".format(p=person)
