...
class PeriodeSoumissionTicketDigitInMemoryTranslator(IPeriodeSoumissionTicketDigitTranslator):

    def get_periodes_actives(cls) -> List['PeriodeSoumissionTicketDigit']:
        return [
            PeriodeSoumissionTicketDigit(
                annee=2023,
                date_debut=datetime.datetime.now() - timedelta(days=1),
                date_fin=datetime.datetime.now() + timedelta(days=10),
            )
        ]
