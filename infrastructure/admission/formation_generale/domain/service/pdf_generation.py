...
from admission.utils import WeasyprintStylesheets, get_portal_admission_url
...
class PDFGeneration(IPDFGeneration):
    ...
    def generer_attestation_refus_sic(
        ...
        if proposition.type_de_refus == TypeDeRefus.REFUS_ARTICLE_95_CONTINGENTES_NR_DOSSIER_INCOMPLET.name:
            ...
            documents_reclames = [
                document_reclame
                for document_reclame in documents_reclames
    ...
    @classmethod
    def generer_acceptation_contingente(
        cls,
        proposition_repository: IPropositionRepository,
        profil_candidat_translator: IProfilCandidatTranslator,
        proposition: PropositionContingenteResume,
    ) -> None:
        answer_deadline_calendar = (
            AcademicCalendar.objects.filter(
        ...
        with translation.override(settings.LANGUAGE_CODE_FR):
            ...
            documents_demandes = []
            for document_id, document in proposition_dto.documents_demandes.items():
                if document_id in ALL_DOCUMENTS_LABEL:
                    ...
                    try:
                        ...
                        libelle = admission_form_item.title.get('fr-be', _('Document'))
            ...
            token = admission_generate_pdf(
                admission=None,
                template='admission/exports/contingente_acceptation.html',
                filename=f'Notification d’acceptation {proposition_dto.reference}.pdf',
                context={
                    'proposition': proposition_dto,
                    'profil_candidat_identification': profil_candidat_identification,
                    'director': cls._get_sic_director(proposition_dto),
                    'drawing_calendar': drawing_calendar,
                    'answer_deadline_calendar': answer_deadline_calendar,
                    'documents_demandes': documents_demandes,
                    'admission_link_front': get_portal_admission_url('general-education', proposition.entity_id.uuid)
                    + 'update/documents',
                },
            )
        proposition.acceptation_contingente = [token]
