...
class ContingenteTrainingImportView(ContingenteMixin, PermissionRequiredMixin, FormView):
    ...
    def import_wb(self, wb: Workbook):
        ...
        for line in lines:
            ...
            if line[10] and line[11]:
                GeneralEducationAdmission.objects.filter(ares_application_number=line[10]).update(draw_number=int(line[11]))
    ...
    def form_valid(self, form):
        ...
        try:
            ...
            messages.error(self.request, _("There was an error while importing the file: {error}.").format(error=str(e)))
