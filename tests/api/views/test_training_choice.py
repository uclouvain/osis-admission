...
from admission.ddd.admission.enums import TypeSituationAssimilation, ChoixAssimilation1, ChoixAssimilation2, \
    ChoixAssimilation3, LienParente, ChoixAssimilation5, ChoixAssimilation6, ChoixAffiliationSport, \
...
class DoctorateAdmissionTrainingChoiceInitializationApiTestCase(APITestCase):
    ...
    def test_admission_doctorate_creation_based_on_pre_admission(self):
        ...
        # Accounting
        # Absence debts
        pre_admission.accounting.institute_absence_debts_certificate = self.documents_tokens['institute_absence_debts_certificate']

        # Assimilation
        pre_admission.accounting.assimilation_situation = TypeSituationAssimilation.A_BOURSE_ARTICLE_105_PARAGRAPH_2.name
        ...
        pre_admission.accounting.cire_unlimited_stay_foreigner_card = self.documents_tokens['cire_unlimited_stay_foreigner_card']
        pre_admission.accounting.ue_family_member_residence_card = self.documents_tokens['ue_family_member_residence_card']
        pre_admission.accounting.ue_family_member_permanent_residence_card = self.documents_tokens['ue_family_member_permanent_residence_card']
        ...
        pre_admission.accounting.refugees_stateless_annex_25_26 = self.documents_tokens['refugees_stateless_annex_25_26']
        ...
        pre_admission.accounting.subsidiary_protection_decision = self.documents_tokens['subsidiary_protection_decision']
        ...
        pre_admission.accounting.assimilation_3_situation_type = ChoixAssimilation3.AUTORISATION_SEJOUR_ET_REVENUS_DE_REMPLACEMENT.name
        pre_admission.accounting.professional_3_month_residence_permit = self.documents_tokens['professional_3_month_residence_permit']
        pre_admission.accounting.salary_slips = self.documents_tokens['salary_slips']
        pre_admission.accounting.replacement_3_month_residence_permit = self.documents_tokens['replacement_3_month_residence_permit']
        pre_admission.accounting.unemployment_benefit_pension_compensation_proof = self.documents_tokens['unemployment_benefit_pension_compensation_proof']
        ...
        pre_admission.accounting.household_composition_or_birth_certificate = self.documents_tokens['household_composition_or_birth_certificate']
        pre_admission.accounting.tutorship_act = self.documents_tokens['tutorship_act']
        pre_admission.accounting.household_composition_or_marriage_certificate = self.documents_tokens['household_composition_or_marriage_certificate']
        pre_admission.accounting.legal_cohabitation_certificate = self.documents_tokens['legal_cohabitation_certificate']
        pre_admission.accounting.parent_identity_card = self.documents_tokens['parent_identity_card']
        pre_admission.accounting.parent_long_term_residence_permit = self.documents_tokens['parent_long_term_residence_permit']
        pre_admission.accounting.parent_refugees_stateless_annex_25_26_or_protection_decision = self.documents_tokens['parent_refugees_stateless_annex_25_26_or_protection_decision']
        pre_admission.accounting.parent_3_month_residence_permit = self.documents_tokens['parent_3_month_residence_permit']
        ...
        pre_admission.accounting.assimilation_6_situation_type = ChoixAssimilation6.A_BOURSE_COOPERATION_DEVELOPPEMENT.name
        ...
        pre_admission.accounting.ue_long_term_stay_identity_document = self.documents_tokens['ue_long_term_stay_identity_document']
        ...
        # Accounting
        self.assertEqual(new_admission.accounting.institute_absence_debts_certificate, self.duplicated_documents_tokens['institute_absence_debts_certificate'],)

        self.assertEqual(new_admission.accounting.assimilation_situation, pre_admission.accounting.assimilation_situation)

        self.assertEqual(new_admission.accounting.assimilation_1_situation_type, pre_admission.accounting.assimilation_1_situation_type)
        self.assertEqual(new_admission.accounting.long_term_resident_card, self.duplicated_documents_tokens['long_term_resident_card'],)
        self.assertEqual(new_admission.accounting.cire_unlimited_stay_foreigner_card, self.duplicated_documents_tokens['cire_unlimited_stay_foreigner_card'],)
        self.assertEqual(new_admission.accounting.ue_family_member_residence_card, self.duplicated_documents_tokens['ue_family_member_residence_card'],)
        self.assertEqual(new_admission.accounting.ue_family_member_permanent_residence_card, self.duplicated_documents_tokens['ue_family_member_permanent_residence_card'],)

        self.assertEqual(new_admission.accounting.assimilation_2_situation_type, pre_admission.accounting.assimilation_2_situation_type)
        self.assertEqual(new_admission.accounting.stateless_person_proof, self.duplicated_documents_tokens['stateless_person_proof'],)
        self.assertEqual(new_admission.accounting.refugee_a_b_card, self.duplicated_documents_tokens['refugee_a_b_card'],)
        self.assertEqual(new_admission.accounting.refugees_stateless_annex_25_26, self.duplicated_documents_tokens['refugees_stateless_annex_25_26'],)
        self.assertEqual(new_admission.accounting.registration_certificate, self.duplicated_documents_tokens['registration_certificate'],)
        self.assertEqual(new_admission.accounting.a_b_card, self.duplicated_documents_tokens['a_b_card'],)
        self.assertEqual(new_admission.accounting.subsidiary_protection_decision, self.duplicated_documents_tokens['subsidiary_protection_decision'],)
        self.assertEqual(new_admission.accounting.a_card, self.duplicated_documents_tokens['a_card'],)

        self.assertEqual(new_admission.accounting.assimilation_3_situation_type, pre_admission.accounting.assimilation_3_situation_type)
        self.assertEqual(new_admission.accounting.temporary_protection_decision, self.duplicated_documents_tokens['temporary_protection_decision'],)
        self.assertEqual(new_admission.accounting.professional_3_month_residence_permit, self.duplicated_documents_tokens['professional_3_month_residence_permit'],)
        self.assertEqual(new_admission.accounting.salary_slips, self.duplicated_documents_tokens['salary_slips'],)
        self.assertEqual(new_admission.accounting.replacement_3_month_residence_permit, self.duplicated_documents_tokens['replacement_3_month_residence_permit'],)
        self.assertEqual(new_admission.accounting.unemployment_benefit_pension_compensation_proof, self.duplicated_documents_tokens['unemployment_benefit_pension_compensation_proof'],)

        self.assertEqual(new_admission.accounting.cpas_certificate, self.duplicated_documents_tokens['cpas_certificate'],)

        self.assertEqual(new_admission.accounting.relationship, pre_admission.accounting.relationship)
        self.assertEqual(new_admission.accounting.assimilation_5_situation_type, pre_admission.accounting.assimilation_5_situation_type)
        self.assertEqual(new_admission.accounting.household_composition_or_birth_certificate, self.duplicated_documents_tokens['household_composition_or_birth_certificate'],)
        self.assertEqual(new_admission.accounting.tutorship_act, self.duplicated_documents_tokens['tutorship_act'],)
        self.assertEqual(new_admission.accounting.household_composition_or_marriage_certificate, self.duplicated_documents_tokens['household_composition_or_marriage_certificate'],)
        self.assertEqual(new_admission.accounting.legal_cohabitation_certificate, self.duplicated_documents_tokens['legal_cohabitation_certificate'],)
        self.assertEqual(new_admission.accounting.parent_identity_card, self.duplicated_documents_tokens['parent_identity_card'],)
        self.assertEqual(new_admission.accounting.parent_long_term_residence_permit, self.duplicated_documents_tokens['parent_long_term_residence_permit'],)
        self.assertEqual(new_admission.accounting.parent_refugees_stateless_annex_25_26_or_protection_decision, self.duplicated_documents_tokens['parent_refugees_stateless_annex_25_26_or_protection_decision'],)
        self.assertEqual(new_admission.accounting.parent_3_month_residence_permit, self.duplicated_documents_tokens['parent_3_month_residence_permit'],)
        self.assertEqual(new_admission.accounting.parent_salary_slips, self.duplicated_documents_tokens['parent_salary_slips'],)
        self.assertEqual(new_admission.accounting.parent_cpas_certificate, self.duplicated_documents_tokens['parent_cpas_certificate'],)

        self.assertEqual(new_admission.accounting.assimilation_6_situation_type, pre_admission.accounting.assimilation_6_situation_type)
        self.assertEqual(new_admission.accounting.cfwb_scholarship_decision, self.duplicated_documents_tokens['cfwb_scholarship_decision'],)
        self.assertEqual(new_admission.accounting.scholarship_certificate, self.duplicated_documents_tokens['scholarship_certificate'],)

        self.assertEqual(new_admission.accounting.ue_long_term_stay_identity_document, self.duplicated_documents_tokens['ue_long_term_stay_identity_document'],)
        self.assertEqual(new_admission.accounting.belgium_residence_permit, self.duplicated_documents_tokens['belgium_residence_permit'],)
        ...
        self.assertEqual(new_admission.accounting.other_format_account_number, pre_admission.accounting.other_format_account_number)
        self.assertEqual(new_admission.accounting.bic_swift_code, pre_admission.accounting.bic_swift_code)
        self.assertEqual(new_admission.accounting.account_holder_first_name, pre_admission.accounting.account_holder_first_name)
        self.assertEqual(new_admission.accounting.account_holder_last_name, pre_admission.accounting.account_holder_last_name)
