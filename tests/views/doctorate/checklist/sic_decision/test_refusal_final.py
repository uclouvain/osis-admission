...
from admission.templatetags.admission import MONS, SAINT_LOUIS
...
from base.models.campus import LOUVAIN_LA_NEUVE_CAMPUS_NAME, Campus
...
class SicRefusalFinalDecisionViewTestCase(SicPatchMixin, TestCase):
    ...
    def setUpTestData(cls):
        ...
        cls.mons_training = DoctorateFactory(
            management_entity=cls.first_doctoral_commission,
            academic_year=cls.academic_years[0],
            enrollment_campus=CampusFactory(
                name=MONS,
        ...
        cls.url = resolve_url(
            'admission:doctorate:sic-decision-refusal-final',
    ...
    def test_refusal_final_decision_form_initialization(self):
        ...
        body_value = form['body'].value()

        self.assertTrue(subject_value)
        self.assertIn(str(self.admission.reference), subject_value)

        self.assertTrue(body_value)
        self.assertIn(f'{self.admission.candidate.first_name} {self.admission.candidate.last_name}', body_value)
        self.assertIn(f'{self.sic_manager_user.person.first_name} {self.sic_manager_user.person.last_name}', body_value)
        self.assertIn('SERA_AUTOMATIQUEMENT_REMPLACE_PAR_LE_LIEN', body_value)
        self.assertIn(f'{self.admission.training.acronym} / {self.admission.training.title}', body_value)
    ...
    def test_refusal_final_decision_form_submitting_for_a_mons_training(self):
        ...
        self.admission.training = self.mons_training
        self.admission.save(update_fields=['training'])
