...
class PastExperiencesAccessTitleEquivalencyViewTestCase(SicPatchMixin, TestCase):
    ...
    def setUpTestData(cls):
        ...
        cls.fac_manager_user = ProgramManagerRoleFactory(education_group=cls.training.education_group).person.user
        cls.default_headers = {'HTTP_HX-Request': 'true'}
        cls.url_name = 'admission:doctorate:past-experiences-access-title-equivalency'
        cls.default_data = {
    ...
    def test_submit_form_with_valid_equivalency_type(self):
        ...
        for equivalency_type in [
            ...
            # Check the admission
            self.admission.refresh_from_db()
            self.assertEqual(self.admission.foreign_access_title_equivalency_type, equivalency_type)
            self.assertEqual(
                self.admission.foreign_access_title_equivalency_status,
        ...
        for equivalency_type in [
            ...
            self.admission.refresh_from_db()
            self.assertEqual(self.admission.foreign_access_title_equivalency_type, equivalency_type)
            self.assertEqual(self.admission.foreign_access_title_equivalency_status, '')
            self.assertEqual(self.admission.foreign_access_title_equivalency_state, '')
            self.assertEqual(self.admission.foreign_access_title_equivalency_effective_date, None)
    ...
    def test_submit_form_with_valid_equivalency_status(self):
        ...
        for equivalency_status in [
            ...
            self.admission.refresh_from_db()
            self.assertEqual(
                self.admission.foreign_access_title_equivalency_type,
                TypeEquivalenceTitreAcces.EQUIVALENCE_CESS.name,
            )
            self.assertEqual(self.admission.foreign_access_title_equivalency_status, equivalency_status)
            self.assertEqual(
                self.admission.foreign_access_title_equivalency_state, EtatEquivalenceTitreAcces.DEFINITIVE.name
            )
            self.assertEqual(self.admission.foreign_access_title_equivalency_effective_date, datetime.date(2023, 1, 1))
    ...
    def test_submit_form_with_valid_equivalency_state(self):
        ...
        for equivalency_state in [
            ...
            self.assertEqual(self.admission.foreign_access_title_equivalency_state, equivalency_state)
            self.assertEqual(self.admission.foreign_access_title_equivalency_effective_date, datetime.date(2023, 1, 1))
...
>>>>>>> 3a6a2be13 (Fix OS-1340 & OS-1343: [V1.B] Front-office: Parcours antérieur > Sous-onglet Examens : concours d'entrée en médecine & examen d'admission d'ingénieur civil & [V1.B] Front-office : Etudes secondaires pour inscription à bachelier > Examen d'accès au premier cycle > Ajout d'une année)
