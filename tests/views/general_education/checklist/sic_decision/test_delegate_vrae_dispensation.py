...
class SicDecisionDispensationViewTestCase(SicPatchMixin, TestCase):
    ...
    def setUpTestData(cls):
        ...
        cls.url = resolve_url(
            'admission:general-education:sic-decision-delegate-vrae-dispensation',
    ...
    def test_post_as_sic_user(self):
        ...
        response = self.client.post(
            self.url,
            data={
                'dispensation': BesoinDeDerogationDelegueVrae.DEROGATION_VRAE.name,
                'certificate_0': [uuid.uuid4()],
                'comment': 'comment',
            },
        ...
        self.assertEqual(
            self.general_admission.delegate_vrae_dispensation, BesoinDeDerogationDelegueVrae.DEROGATION_VRAE.name
        )
        self.assertEqual(self.general_admission.delegate_vrae_dispensation_comment, 'comment')
        self.assertEqual(self.general_admission.delegate_vrae_dispensation_certificate, [self.file_uuid])
