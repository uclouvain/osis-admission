...
class FacultyDecisionSendToSicViewTestCase(TestCase):
    ...
    def test_send_to_sic_with_fac_user_in_specific_statuses_to_approve_with_secondary_studies_as_access_title(
        ...
        self.assertEqual(
            ...
            f'{diploma_year}-{diploma_year + 1} : {secondary_studies_base_title}',
        )
...
@override_settings(OSIS_DOCUMENT_BASE_URL='http://dummyurl')
class CourseChangeFacultyApprovalDecisionViewTestCase(TestCase):
    @classmethod
    def setUpTestData(cls):
        cls.academic_years = [AcademicYearFactory(year=year) for year in [2021, 2022]]
    ...
    def setUp(self) -> None:
        ...
        self.default_checklist = copy.deepcopy(self.general_admission.checklist)
        self.url = resolve_url(
            'admission:general-education:course-change-fac-decision-approval',
            uuid=self.general_admission.uuid,
        )
        self.file_uuid = uuid.UUID('4bdffb42-552d-415d-9e4c-725f10dce228')
    ...
    def test_approval_decision_form_submitting_with_invalid_context(self):
        ...
        self.assertDjangoMessage(
            response=response,
            message=gettext(
                'The proposition must involve a change of course and must have a defined access condition.',
            ),
        )

        # No late enrollment
        self.general_admission.is_external_reorientation = False
        self.general_admission.admission_requirement = ConditionAcces.BAC.name
        self.general_admission.save()
        ...
        # No selected access title
        self.general_admission.is_external_reorientation = True
        self.general_admission.save()
