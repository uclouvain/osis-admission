...
class ReplaceDocumentTestCase(BaseDocumentViewTestCase):
    ...
    def test_general_sic_manager_replaces_a_document(self, frozen_time):
        ...
        response = self.client.post(
            resolve_url(base_url, uuid=self.general_admission.uuid, identifier=self.sic_free_requestable_document),
            data={
                'file_0': ['file_0-token'],
            },
            **self.default_headers,
        ...
        self.assertEqual(
            len(self.general_admission.get_specific_question_answers_dict().get(specific_question_uuid)), 1
        )
    ...
    def test_general_fac_manager_replaces_a_document(self, frozen_time):
        ...
        response = self.client.post(
            resolve_url(base_url, uuid=self.general_admission.uuid, identifier=self.fac_free_requestable_document),
            data={
                'file_0': ['file_0-token'],
            },
            **self.default_headers,
    ...
    @freezegun.freeze_time('2022-01-01', as_kwarg='frozen_time')
    def test_continuing_sic_manager_replaces_a_document(self, frozen_time):
        self.init_documents(for_sic=True, admission=self.continuing_admission)

        base_url = 'admission:continuing-education:document:replace'
        ...
        for identifier in [
            # A SIC manager cannot system documents
            f'{IdentifiantBaseEmplacementDocument.SYSTEME.name}.DOSSIER_ANALYSE',
        ]:
            response = self.client.post(
        ...
        # A SIC manager can replace FAC and SIC documents
        for identifier in [
            self.fac_free_non_requestable_internal_document,
            self.fac_free_requestable_candidate_document_with_default_file,
            self.fac_free_requestable_document,
        ...
        self.assertEqual(
            len(self.continuing_admission.get_specific_question_answers_dict().get(specific_question_uuid)), 1
        )
    ...
    @freezegun.freeze_time('2022-01-01', as_kwarg='frozen_time')
    def test_continuing_fac_manager_replaces_a_document(self, frozen_time):
        ...
        for identifier in [
            # A FAC manager cannot system documents
            f'{IdentifiantBaseEmplacementDocument.SYSTEME.name}.DOSSIER_ANALYSE',
        ]:
            response = self.client.post(
                resolve_url(base_url, uuid=self.continuing_admission.uuid, identifier=identifier),
        ...
        self.change_remote_metadata_patcher.reset_mock()
        old_document_uuid = uuid.UUID(self.fac_free_non_requestable_internal_document.split('.')[-1])
        self.assertIn(old_document_uuid, self.continuing_admission.uclouvain_fac_documents)
        self.assertEqual(len(self.continuing_admission.uclouvain_fac_documents), 2)
        response = self.client.post(
            resolve_url(
                base_url,
        ...
        self.assertEqual(response.status_code, 200)
        self.continuing_admission.refresh_from_db()
        self.assertNotIn(old_document_uuid, self.continuing_admission.uclouvain_fac_documents)
        self.assertEqual(len(self.continuing_admission.uclouvain_fac_documents), 2)
    ...
    @freezegun.freeze_time('2022-01-01', as_kwarg='frozen_time')
    def test_doctorate_sic_manager_replaces_a_document(self, frozen_time):
        self.init_documents(for_sic=True, admission=self.doctorate_admission)

        base_url = 'admission:doctorate:document:replace'
        ...
        response = self.client.post(
            resolve_url(base_url, uuid=self.doctorate_admission.uuid, identifier=self.sic_free_requestable_document),
            data={
                'file_0': ['file_0-token'],
            },
            **self.default_headers,
        ...
        self.assertEqual(
            len(self.doctorate_admission.get_specific_question_answers_dict().get(specific_question_uuid)), 1
        )
    ...
    @freezegun.freeze_time('2022-01-01', as_kwarg='frozen_time')
    def test_doctorate_fac_manager_replaces_a_document(self, frozen_time):
        ...
        response = self.client.post(
            resolve_url(base_url, uuid=self.doctorate_admission.uuid, identifier=self.fac_free_requestable_document),
            data={
                'file_0': ['file_0-token'],
            },
            **self.default_headers,
