...
class IncompletePersonForBachelorFactory(CompletePersonFactory):
    ...
    @factory.post_generation
    def create_related_objects(self, create, extracted, **kwargs):
        HighSchoolDiplomaFactory(person=self, got_diploma=GotDiploma.NO.name, academic_graduation_year=None)
        PersonAddressFactory(
            person=self,
            label=PersonAddressType.RESIDENTIAL.name,
            street='University street',
...
class InternalPersonFactory(PersonFactory):
    email = factory.Faker('email', domain=settings.INTERNAL_EMAIL_SUFFIX)
