...
def migrate_specific_answers(apps, schema_editor):
    ...
    form_items = {
        ...
        for form_item in AdmissionFormItem.objects.values('uuid', 'id', 'type')
    }
    total = BaseAdmission.objects.exclude(old_specific_question_answers={}).count()
    answers = []
    for i, admission in enumerate(BaseAdmission.objects.exclude(old_specific_question_answers={}).only('id', 'old_specific_question_answers')):
        for form_item_uuid, answer in admission.old_specific_question_answers.items():
            ...
            if form_item is None:
                print(f'Unknown form item UUID {form_item_uuid} in admission {admission}')
                continue
            answers.append(SpecificQuestionAnswer(
                admission=admission,
                form_item_id=form_item['id'],
                file=answer if form_item['type'] == 'DOCUMENT' else None,
        ...
        if i % 1000 == 0:
            print(f'{i} / {total}')
    print(f'{total} / {total}\nBulk creating in DB...')
    SpecificQuestionAnswer.objects.bulk_create(answers)
    ...
    operations = [
        ...
        migrations.AddConstraint(
            ...
            constraint=models.CheckConstraint(
                check=models.Q(
                    ("file__isnull", False), ("answer__isnull", False), _connector="OR"
                ),
