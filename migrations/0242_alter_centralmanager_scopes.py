...
def _update_central_manager_scopes(apps, old_value, new_value):
    ...
    central_managers_with_old_scope = CentralManager.objects.filter(scopes__contains=[old_value])

    for central_manager in central_managers_with_old_scope:
        central_manager.scopes[central_manager.scopes.index(old_value)] = new_value

    CentralManager.objects.bulk_update(central_managers_with_old_scope, ['scopes'])
...
class Migration(migrations.Migration):
    ...
    dependencies = [
        ('admission', '0241_doctorateadmission_related_pre_admission'),
    ]
