...
from osis_profile.models import EXAM_TYPE_PREMIER_CYCLE_LABEL_FR, ExamType
...
json_experience_condition = Value(
    '$.current.parcours_anterieur.enfants[*] ? '
    '(@.statut == $target_status && @.extra.identifiant == $experience_identifier)'
)


def in_draft_educational_experiences_qs(
    educational_experience_model,
    epc_injection_model,
    admission_educational_valuated_experiences_model,
):
    """
    Return the educational experiences whose the validation status must be "In draft"
    """
    return educational_experience_model.objects.filter(external_id__isnull=True).filter(
...
def validated_educational_experiences_qs(
    educational_experience_model,
    admission_educational_valuated_experiences_model,
    inscription_programme_annuel_model,
):
    """
    Return the educational experiences whose the validation status must be "Validated"
    """
    return (
        educational_experience_model.objects.exclude(
...
def in_draft_professional_experiences_qs(
    professional_experience_model,
    epc_injection_model,
    admission_professional_valuated_experiences_model,
):
    """
    Return the professional experiences whose the validation status must be "In draft"
    """
...
def validated_professional_experiences_qs(
    professional_experience_model,
    admission_professional_valuated_experiences_model,
    inscription_programme_annuel_model,
):
    """
    Return the professional experiences whose the validation status must be "Validated"
    """
    return (
        professional_experience_model.objects.exclude(
            validation_status=ChoixStatutValidationExperience.EN_BROUILLON.name,
        )
        .annotate(
            valid_target_json=JSONObject(
...
def in_draft_secondary_studies_qs(
    ...
    first_cycle_exam_type_id,
):
    """
    Return the secondary studies whose the validation status must be "In draft"
    """
    return secondary_studies_model.objects.filter(
        ~Exists(
            foreign_high_school_diploma_model.objects.filter(
                person_id=OuterRef('person_id'),
                external_id__isnull=False,
            )
        ),
        ~Exists(
            belgian_high_school_diploma_model.objects.filter(
                person_id=OuterRef('person_id'),
                external_id__isnull=False,
            )
        ),
        ~Exists(
            exam_model.objects.filter(
                person_id=OuterRef('person_id'),
                external_id__isnull=False,
        ...
        ~Exists(
            epc_injection_model.objects.filter(
                person_id=OuterRef('person_id'),
...
def validated_secondary_studies_qs(
    secondary_studies_model,
    foreign_high_school_diploma_model,
    belgian_high_school_diploma_model,
    exam_model,
    ...
    """
    Return the secondary studies whose the validation status must be "Validated"
    """
    return secondary_studies_model.objects.exclude(
        validation_status=ChoixStatutValidationExperience.EN_BROUILLON.name,
    ).filter(
        Exists(
            foreign_high_school_diploma_model.objects.filter(person_id=OuterRef('person_id'), external_id__isnull=False)
        )
        | Exists(
            belgian_high_school_diploma_model.objects.filter(person_id=OuterRef('person_id'), external_id__isnull=False)
        )
        | Exists(
            exam_model.objects.filter(
                person_id=OuterRef('person_id'),
...
def in_draft_exams_qs(exam_model, admission_exam_model, first_cycle_exam_type_id):
    """
    Return the exams whose the validation status must be "In draft"
    """
    return (
        exam_model.objects.exclude(
            type_id=first_cycle_exam_type_id,
        )
        .filter(
            external_id__isnull=True,
...
def validated_exams_qs(exam_model, admission_exam_model, first_cycle_exam_type_id):
    """
    Return the exams whose the validation status must be "Validated"
    """
    return (
        exam_model.objects.exclude(type_id=first_cycle_exam_type_id)
        .exclude(validation_status=ChoixStatutValidationExperience.EN_BROUILLON.name)
        .filter(
            Q(external_id__isnull=False)
            | Exists(
                admission_exam_model.objects.filter(
                    exam_id=OuterRef('pk'),
                    admission__generaleducationadmission__status=(
...
def extract_experience_authentication_status(
    base_admission_model,
    educational_experience_model,
    exam_model,
    professional_experience_model,
    ...
    """
    Extract the checklist authentication status from the admission to the experience
    """
    ...
    authentication_states_to_update = (
        # The order is important to set the priority (VRAI > FAUX > ETABLISSEMENT_CONTACTE > AUTHENTIFICATION_DEMANDEE)
        ...
        for experience_checklist in admission.checklist['current']['parcours_anterieur']['enfants']:
            ...
            if experience_checklist['extra'].get('etat_authentification') in authentication_states_to_update:
                if experience_checklist['extra']['identifiant'] == 'ETUDES_SECONDAIRES':
                    ...
                    ].add(admission.candidate_id)
                else:
                    experiences_uuids_by_authentication_status[
                        experience_checklist['extra']['etat_authentification']
                    ].add(experience_checklist['extra']['identifiant'])
    ...
    secondary_studies_model.objects.filter(person_id__in=candidates_ids_with_authenticated_secondary_studies).update(
        authentication_status=Case(*authenticated_secondary_studies_conditions)
    )

    for model in [exam_model, educational_experience_model, professional_experience_model]:
        model.objects.filter(uuid__in=authenticated_experiences_uuids).update(
            authentication_status=authenticated_experiences_case
        )


def update_first_cycle_exam_statuses(
    exam_model,
    secondary_studies_model,
    first_cycle_exam_type_id,
):
    """
    The validation and the authentication statuses are duplicated for the first cycle exam (Exam vs HighSchoolDiploma)
    so we harmonize them
    """
    matched_secondary_studies = secondary_studies_model.objects.filter(person_id=OuterRef('person_id'))

    return exam_model.objects.filter(
        type_id=first_cycle_exam_type_id,
        person__highschooldiploma__isnull=False,
    ).update(
        validation_status=Subquery(matched_secondary_studies.values('validation_status')[:1]),
        authentication_status=Subquery(matched_secondary_studies.values('authentication_status')[:1]),
    )
...
def extract_experience_checklist(
    ...
    first_cycle_exam_type_ids = ExamType.objects.filter(label_fr=EXAM_TYPE_PREMIER_CYCLE_LABEL_FR).values_list(
        'pk',
        flat=True,
    )
