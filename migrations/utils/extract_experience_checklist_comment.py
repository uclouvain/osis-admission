...
def extract_experience_checklist_comment(comment_model, base_admission_model):
    ...
    matching_admission = base_admission_model.objects.filter(
        uuid=OuterRef('object_uuid'),
        candidate__highschooldiploma__isnull=False,
    )
    comment_model.objects.filter(
        ...
        tags=ArrayReplace(
            'tags',
            Value('ETUDES_SECONDAIRES'),
            Cast('high_school_diploma_uuid', output_field=CharField()),
        ),
    )

    # Update the object uuid to specify the exam uuid instead of the admission uuid (old comments related to exams)
    matching_admission = base_admission_model.objects.filter(uuid=OuterRef('object_uuid'), exam__isnull=False)
    comment_model.objects.filter(
        tags__0='parcours_anterieur',
        tags__1='EXAMS',
    ).annotate(
        exam_uuid=Subquery(matching_admission.values('exam__exam__uuid')[:1]),
    ).filter(exam_uuid__isnull=False).update(
        object_uuid=F('exam_uuid'),
        tags=ArrayReplace('tags', Value('EXAMS'), Cast('exam_uuid', output_field=CharField())),
    )

    # Now we can have several comments by object uuid by tags but we want only unique comments so we merge them
    duplicate_comments = (
        comment_model.objects.annotate(tags_1_len=Length('tags__1'))
        .filter(experience_condition)
        .filter(
            Exists(
                comment_model.objects.filter(
                    tags=OuterRef('tags'),
    ...
    for comment in duplicate_comments:
        ...
        comment_content = (
            f'{comment_author} ({comment.modified_at:%H:%M} - {comment.modified_at:%d/%m/%Y}) : {comment.content}\n'
        )
