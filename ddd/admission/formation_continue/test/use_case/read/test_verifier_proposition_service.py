...
class TestVerifierPropositionService(TestCase):
    ...
    def setUp(self) -> None:
        ...
        self.formation_continue_translator.reset()
    ...
    def test_should_retourner_erreur_si_moyens_decouverte_formation_non_renseignes_formation_longue(self):
        training_info = next(
            training
            for training in self.formation_continue_translator.iufc_specific_informations_trainings_repository._informations_specifiques
            if training.entity_id.sigle_formation == 'USCC1' and training.entity_id.annee == 2020
        )

        with mock.patch.multiple(training_info, inscription_au_role_obligatoire=True):
            with mock.patch.multiple(
                self.complete_proposition,
    ...
    def test_should_etre_ok_si_moyens_decouverte_formation_non_renseignes_et_infos_formation_non_trouvees(self):
        self.formation_continue_translator.iufc_specific_informations_trainings = []
