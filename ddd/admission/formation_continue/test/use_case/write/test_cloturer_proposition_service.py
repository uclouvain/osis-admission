...
class CloturerPropositionTestCase(SimpleTestCase):
    def setUp(self) -> None:
        self.proposition_repository = PropositionInMemoryRepository()
        self.addCleanup(self.proposition_repository.reset)
        self.message_bus = message_bus_in_memory_instance
        self.cmd = CloturerPropositionCommand(
            uuid_proposition='uuid-USCC2',
            gestionnaire="gestionnaire",
        )

    def test_should_cloturer(self):
        ...
        self.assertEqual(proposition.statut, ChoixStatutPropositionContinue.CLOTUREE)
        self.assertEqual(proposition.checklist_actuelle.decision.statut, ChoixStatutChecklist.GEST_BLOCAGE)
        self.assertDictEqual(proposition.checklist_actuelle.decision.extra, {'blocage': "closed"})
