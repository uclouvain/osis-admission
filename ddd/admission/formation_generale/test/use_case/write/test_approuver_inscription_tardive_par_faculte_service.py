...
from admission.ddd.admission.formation_generale.commands import ApprouverInscriptionTardiveParFaculteCommand
...
class TestApprouverInscriptionTardiveParFaculte(TestCase):
    @classmethod
    def setUpClass(cls) -> None:
        super().setUpClass()
        cls.proposition_repository = PropositionInMemoryRepository()
        cls.message_bus = message_bus_in_memory_instance
        cls.command = ApprouverInscriptionTardiveParFaculteCommand
    ...
    def test_should_lever_exception_si_inscription_non_tardive(self):
        self.proposition.est_inscription_tardive = None
        with self.assertRaises(MultipleBusinessExceptions) as context:
            self.message_bus.invoke(self.command(**self.parametres_commande_par_defaut))
            self.assertIsInstance(context.exception.exceptions.pop(), InscriptionTardiveAvecConditionAccesException)

        self.proposition.est_inscription_tardive = False
        with self.assertRaises(MultipleBusinessExceptions) as context:
            self.message_bus.invoke(self.command(**self.parametres_commande_par_defaut))
            self.assertIsInstance(context.exception.exceptions.pop(), InscriptionTardiveAvecConditionAccesException)
