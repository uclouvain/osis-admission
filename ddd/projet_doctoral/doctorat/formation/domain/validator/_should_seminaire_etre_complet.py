...
from admission.ddd.projet_doctoral.doctorat.formation.dtos import SeminaireDTO
from base.ddd.utils.business_validator import BusinessValidator


@attr.dataclass(frozen=True, slots=True)
class ShouldSeminaireEtreComplet(BusinessValidator):
    ...
    def validate(self, *args, **kwargs):
        if not all(
            [
                self.seminaire.type,
                self.seminaire.nom,
                self.seminaire.date_debut,
                self.seminaire.date_fin,
                self.seminaire.attestation_participation,
                self.seminaire.volume_horaire,
                self.activite.ects,
            ]
        ):
