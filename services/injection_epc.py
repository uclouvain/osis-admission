...
class InjectionEPCAdmission:
    ...
    def _recuperer_informations_utiles_documents_manquants(cls, type_document_compose):
        ...
        if parties_type_document[0] == 'LIBRE_CANDIDAT':
            ...
            print(question.title['en'])
            document = CategorizedFreeDocument.objects.filter(
                long_label_en__in=[question.title['en'], f"{question.title['en'].split(':')[0]}: " + "{annee_academique}"]
            ).first()
            type_document = unidecode(document.short_label_fr.replace(' ', '_').upper()) if document else unidecode(question.title['en'].replace(' ', '_'))
