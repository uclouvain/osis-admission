...
from admission.ddd.admission.formation_generale.domain.model.enums import PoursuiteDeCycle, BesoinDeDerogation, \
    DroitsInscriptionMontant, TypeDeRefus, ChoixStatutChecklist
from admission.forms import (
    ...
    get_example_text,
    CustomDateInput,
    EMPTY_CHOICE_AS_LIST, EMPTY_CHOICE,
)
...
class SicDecisionApprovalForm(forms.ModelForm):
    ...
    class Meta:
        ...
        widgets = {
            ...
            'program_planned_years_number': forms.Select(
                choices=EMPTY_CHOICE_AS_LIST
                        + [(number, number) for number in
                           range(DUREE_MINIMALE_PROGRAMME, DUREE_MAXIMALE_PROGRAMME + 1)],
            ),
    ...
    def __init__(
            self, academic_year, additional_approval_conditions_for_diploma,
            documents: List[EmplacementDocumentDTO], *args, **kwargs
    ):
        instance: Optional[GeneralEducationAdmission] = kwargs.get('instance', None)
        data = kwargs.get('data', {})
